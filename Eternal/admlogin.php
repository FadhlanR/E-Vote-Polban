...
if(isset($login)){
  ...
  $rs = $db->prepare("select id_admin from admin where username_admin= :username_admin and password_admin= :password_admin");
  $rs->bindValue(':username_admin', $user);
  $rs->bindValue(':password_admin', MD5($pass));
  $rs->execute();

  $query = $rs->fetch(PDO::FETCH_ASSOC);

  //$query = "select id_admin from admin where username_admin='$user' and password_admin= MD5('$pass')";
	if($query['id_admin']!=NULL)
    {
